...
pub(super) fn decode(data: &[u8]) -> Vec<u8> {
    // This list contains all the possible values that can be found in the output data. As we
    // encounter each byte in input, we use it as an index into this list in this list and the
    // value at that index is encoded into the output. Then the value is move to the front of the
    // list.
    let mut symbols: Vec<u8> = (0..=255).collect();
    let mut output = vec![];
